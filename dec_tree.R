...
library(rpart)
model <- rpart(AVGFPS ~ ., data = data[, c(features, target)], method = "anova")

# Step 4: Split dataset into training and testing sets
library(caret)
set.seed(123) # for reproducibility
trainIndex <- createDataPartition(data$AVGFPS, p = 0.7, list = FALSE)
training <- data[trainIndex, ]
testing <- data[-trainIndex, ]

# Step 5: Train the decision tree model
model <- rpart(AVGFPS ~ ., data = training[, c(features, target)], method = "anova")
...
accuracy <- sqrt(mean((predictions - testing$AVGFPS)^2))
...
#legend(x = 8, y = 54, legend = c("Actual", "Predicted"), col = c("blue", "red"), lty = 1)
